...
-  ``GO111MODULE=on go get github.com/terraform-providers/terraform-provider-ucloud``
   (where ``ucloud`` is the name of the provider)
...
The following configuration points are available for the ``ucloud``
provider:

-  ``ucloud:apiKey`` (environment: ``XYZ_API_KEY``) - the API key for
   ``ucloud``
-  ``ucloud:region`` (environment: ``XYZ_REGION``) - the region in which
   to deploy resources
